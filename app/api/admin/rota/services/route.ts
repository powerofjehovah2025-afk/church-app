...
import type { ServiceInsert, ServiceUpdate } from "@/types/database.types";

/**
 * GET: List services (with optional date range filter)
...
export async function PUT(request: Request) {
  try {
    ...
    if (time !== undefined) {
      updateData.time = time?.trim() || null;
    }

    const admin = createAdminClient();
    const { data: updatedService, error } = await admin
      .from("services")
      .update(updateData)
      .eq("id", id)
      .select()
